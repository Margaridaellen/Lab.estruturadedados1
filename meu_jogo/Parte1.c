...
int main(void) {
    ...
    Bola *bolas = malloc(quantidadeBolas * sizeof(Bola));
        ...
        for (int i = 0; i < quantidadeBolas; i++) {
            bolas[i] = (Bola){ { 100 + i * 20, 100 }, { 2, 2 }, 10, RED };
        }
        InitWindow(800, 600, "Redimensionando Vetor - Simplificado");
        SetTargetFPS(60);

        while (!WindowShouldClose()) {
            if (IsKeyPressed(KEY_SPACE)) {
            quantidadeBolas++;
            bolas = realloc(bolas, quantidadeBolas * sizeof(Bola));
            
        
            bolas[quantidadeBolas - 1] = (Bola){ 
                { GetRandomValue(50, 750), GetRandomValue(50, 550) }, 
                { GetRandomValue(-3, 3), GetRandomValue(-3, 3) }, 
                10, 
                BLUE 
            };
        }
        ...
        BeginDrawing();
            ...
            for (int i = 0; i < quantidadeBolas; i++) {
              bolas[i].pos.x += bolas[i].vel.x;
              bolas[i].pos.y += bolas[i].vel.y;
